...
for (i in 1:3) 
{
  for(k in 1:4)
  {
    ...
    for (j in 1:200)
    {
      ...
      beta=rep(NA,p[k])
      Y=rep(NA,n[k])
      c(X,beta,Y):=data_gen_3(n[k],p[k],p_0[i])
...
save(nmp_1,fdr_1,power_1,nmp_arl_1,fdr_arl_1,power_arl_1,nmp_thrl_1,fdr_thrl_1,power_thrl_1,nmp_lasso_1,fdr_lasso_1,power_lasso_1,file="output-3.RData")
